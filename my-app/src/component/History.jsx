...
function History(){
   ...
   const [loading,setLoading]=useState(false)
    const fetchData=(page,query)=>{
        let url=`https://cute-puce-jackrabbit-robe.cyclic.app/History?_limit=3&_page=${page}`
        if(query){
            url=`https://cute-puce-jackrabbit-robe.cyclic.app/History?_limit=4&_page=${page}&q=${query}`
        }
        setLoading(true)
        axios.get(url)
        .then((res)=>{
...
/>:
       ...
       <Text mb={2}>{item.name}</Text>
        ...
        <Button  bg={'blue.400'}color={'white'}  _hover={{bg: 'blue.500', }}
        onClick={()=>setPage(page+1)}  isDisabled={page===3}>NEXT</Button>
         </Stack>
